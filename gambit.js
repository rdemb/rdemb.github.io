...
*/
const MINDS=[
 { id:'kairos', name:'KAIROS', emblem:'burst', hue:38, elo:1612,
   p:{depth:2,temp:64,risk:0.72,aggr:0.82},
   style:{pl:'Intuicja i tempo. Gra szybko, czuje moment.',en:'Intuition and tempo. Plays fast, feels the moment.',de:'Intuition und Tempo. Spielt schnell, spürt den Moment.'} },
 { id:'loom', name:'LOOM', emblem:'weave', hue:46, elo:1704,
   p:{depth:3,temp:18,risk:0.22,aggr:0.30},
   style:{pl:'Tkacz pozycji. Cierpliwy, splata długie plany.',en:'Weaver of positions. Patient, threads long plans.',de:'Weber der Stellung. Geduldig, knüpft lange Pläne.'} },
 { id:'ember', name:'EMBER', emblem:'flame', hue:24, elo:1538,
   p:{depth:2,temp:82,risk:0.92,aggr:0.95},
   style:{pl:'Ofiarnik. Pali mosty, szuka ataku za wszelką cenę.',en:'The sacrificer. Burns bridges, hunts the attack.',de:'Der Opferer. Verbrennt Brücken, jagt den Angriff.'} },
 { id:'vertex', name:'VERTEX', emblem:'poly', hue:50, elo:1668,
   p:{depth:3,temp:24,risk:0.30,aggr:0.40},
   style:{pl:'Geometra. Liczy strukturę, nie lubi luźnych pól.',en:'The geometer. Counts structure, hates loose squares.',de:'Der Geometer. Zählt Struktur, hasst lose Felder.'} },
 { id:'noctua', name:'NOCTUA', emblem:'radar', hue:40, elo:1731,
   p:{depth:3,temp:14,risk:0.32,aggr:0.52},
   style:{pl:'Nocny kalkulator. Widzi głęboko, gra zimno.',en:'The night calculator. Sees deep, plays cold.',de:'Der Nachtrechner. Sieht tief, spielt kalt.'} },
 { id:'flux', name:'FLUX', emblem:'chaos', hue:32, elo:1487,
   p:{depth:1,temp:96,risk:0.62,aggr:0.55},
   style:{pl:'Chaos. Nieprzewidywalny — czasem genialny, czasem nie.',en:'Chaos. Unpredictable — sometimes brilliant, sometimes not.',de:'Chaos. Unberechenbar — mal genial, mal nicht.'} },
 { id:'atlas', name:'ATLAS', emblem:'fortress', hue:43, elo:1623,
   p:{depth:2,temp:22,risk:0.16,aggr:0.26},
   style:{pl:'Materialista. Trzyma się twardo, broni fortecy.',en:'The materialist. Holds firm, defends the fortress.',de:'Der Materialist. Hält fest, verteidigt die Festung.'} },
 { id:'aria', name:'ARIA', emblem:'ribbon', hue:48, elo:1659,
   p:{depth:2,temp:34,risk:0.42,aggr:0.50},
   style:{pl:'Harmonia. Elegancja i czyste końcówki.',en:'Harmony. Elegance and clean endgames.',de:'Harmonie. Eleganz und saubere Endspiele.'} }
];
...
function Board(opts){
  ...
  self.draw=function(now){
    ...
    ctx.font='600 '+(cell*0.16)+"px 'JetBrains Mono',monospace"; ctx.textBaseline='alphabetic';
...
const appState={ games:1240+((Math.random()*60)|0), tourneyH: 0 };
...
function buildBoards(){
  const host=document.getElementById('boards'); host.innerHTML='';
  const intervals=[1900,2500,2150,2800];
  const used=new Set();
  for(let i=0;i<4;i++){
    let pair; let guard=0; do{ pair=pickPair(); guard++; }while(guard<8 && (used.has(pair[0].id)||used.has(pair[1].id)));
    ...
    const avW=card.querySelector('.player[data-side=w] .av canvas');
    const avB=card.querySelector('.player[data-side=b] .av canvas');
    const board=Board({ canvas:bcanvas, white:pair[0], black:pair[1], interval:intervals[i], animDur:380,
      onMove:(b,m)=>{ updateBoardCard(card,b); maybeBanter(b,m); },
      onEnd:(b,st)=>{ endBoard(card,b,st,used); } });
    board._card=card; board._avW=addEmblem(avW,pair[0]); board._avB=addEmblem(avB,pair[1]);
    boards.push(board);
...
document.getElementById('lang').addEventListener('click',e=>{ const b=e.target.closest('button'); if(b) applyLang(b.dataset.lang); });
...
document.getElementById('theme').addEventListener('click',()=>applyTheme(theme==='dark'?'light':'dark'));
...
document.getElementById('menu').addEventListener('click',()=>{ const t=document.getElementById('stoly'); if(t) scrollTo({top:t.offsetTop-50,behavior:reduce?'auto':'smooth'}); });
...
function boot(){
  ...
  // stats count-up
  countUp(document.getElementById('statGames'), appState.games, 1500);
  countUp(document.getElementById('statTourney'), 312, 1500);
  ...
  setInterval(()=>{ if(Math.random()<0.6){ const m=MINDS[(Math.random()*MINDS.length)|0]; pushEter(m,'idle'); } }, 5200);
