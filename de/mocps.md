...
Was sich geändert hat: Ich habe getestet, ob safe fallback / arbitration Beschleunigung nur dann nutzt, wenn die image-derived Evidenz stark genug ist, statt den acceleration state immer zu verwenden.

Ergebnis: safe fallback erhielt die Kontrollfälle `none_control` und `mild_accel` bei `60/60`, mit after-occlusion assignment `1.000` und identity switch rate `0.000`. Bei `strong_accel` verbesserte es after-occlusion assignment von `0.503` auf `0.800` und senkte switch rate von `0.391` auf `0.151`. `direction_change` ist weiter nicht gelöst: winrate stieg auf `45/60`, aber after-occlusion assignment fiel auf `0.718`, und switch rate stieg auf `0.428`. Der diagnostische oracle selector zeigt verbleibendes Potenzial (`50/60`, after `0.924`, switch `0.111`), wenn der Dynamik-Branch korrekt gewählt wird.

Interpretation: Beschleunigung sollte bedingt genutzt werden, nicht immer. Safe fallback ist eine Teilverbesserung für `strong_accel`, aber `direction_change` braucht weiterhin einen besseren Dynamik-Selektor oder reicheren recurrent state.

Einschränkung: nur Toy-Diagnostik; kein vollständiges trainable Slot Attention; kein Benchmark, SOTA, AGI, Physikverständnis oder allgemeines World Model.

## v0.11 — Noisy reappearance memory stress test

Was sich geändert hat: Ich habe getestet, ob safe-fallback memory noisy oder irreführendes Wiederauftauchen nach Okklusion übersteht.

Ergebnis: safe fallback erreichte in jedem geprüften noise mode `60/60` Winrate. Das bedeutet nicht, dass identity binding stabil war. `false_blob` senkte immediate assignment auf `0.791`, recovery assignment auf `0.659`, erhöhte identity switch rate auf `0.351` und hatte false-component selection `0.276`. Controls, pixel noise, flicker und distractor-bright reappearance behielten recovery assignment `1.000` und switch rate `0.000`.

Interpretation: Der nächste Bottleneck ist observation reliability / re-identification, nicht Memory-Länge oder ein weiterer einfacher Dynamik-Term. Safe fallback braucht einen image-derived reliability head oder leichte appearance memory, um transient false components abzulehnen.
...
Was sich geändert hat: Ich habe die easy-`false_blob`-Schwäche aus v0.12 mit größerer gezielter Stichprobe erneut geprüft (`4 x 16`, 960 Samples pro mode/variant für identity metrics), ohne eine neue Architektur hinzuzufügen.

Ergebnis: Die größere gezielte Stichprobe verwarf eine stabile Regression. `learned_reliability_gate` kam auf easy modes zurück zu recovery `0.980`, switch `0.026` und false-component selection `0.028`. Controls blieben erhalten: recovery `1.000`, switch `0.000`. Representative hard modes verbesserten identity metrics weiter gegenüber `safe_fallback_base`: recovery `0.992` vs `0.748`, switch `0.010` vs `0.256`.

Interpretation: v0.12.1 erhält v0.11.1-level identity metrics auf easy false blobs in diesem gezielten Audit und hält die hard-mode Verbesserung. Confidence/risk wirkt besser als im reduced-sample v0.12 Run, aber confidence calibration bleibt offen. Das bleibt eine lokale Toy-Diagnostik.
...
v0.13 auditiert confidence/risk calibration für observation reliability. Image-derived reliability und disagreement risk signals identifizieren viele false-selection und identity-switch risks im geprüften lokalen Grid und ermöglichen conservative abstention ohne held-out threshold leakage.
...
v0.14.2 kartiert den threshold tradeoff für hold-based risk-aware updates. Im geprüften Grid bleibt
`hold_previous_state/conservative` der zero-switch candidate, aber geringere intervention costs
gehen zulasten von identity safety. Das Ergebnis bleibt eine lokale Toy-Diagnostik.

Es löst weder re-identification noch confidence calibration oder breite noisy-reappearance
robustness.
